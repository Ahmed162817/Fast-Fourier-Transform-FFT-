...
real input_data [0:Number_of_iterations-1][0:N-1];            // Time Domian Vectors Randomized from MATLAB
...
initial begin
    ...
    file1 = $fopen("Input_vector.txt", "r");
    if (!file1) begin
    ...
    file2 = $fopen("Output_real_vector.txt", "r");
    if (!file2) begin
      $display("Error: Could not open file!");
      $finish;
    end
    ...
    file3 = $fopen("Output_imag_vector.txt", "r");
    if (!file3) begin
      $display("Error: Could not open file!");
      $finish;
    end
...
initial begin
    ...
    for (int i = 0 ; i < Number_of_iterations; i++) begin
        ...
        for (int k = 0; k < N; k++) begin
            data_in[(N-k)*WIDTH-1 -: WIDTH] = $rtoi(input_data[i][k] * 256.0);                      // Q8.8 fixed-point Input Format
        end
        repeat ($clog2(N)+1) @(negedge clk);                                                       // we take additional clock cycle as we register the inputs
